...
import { initialBracketState } from './bracketDefaults';
...
export default function TournamentManagerPanel({ rating, userId }) {
  ...
  const [bracketState, setBracketState] = useState(initialBracketState());
  ...
  const loadBracketAssignments = async (tournamentId) => {
    ...
    if (!error && data?.length) {
      setBracketState((prev) => {
        // Deep clone so placeholder labels survive the merge
        const newState = JSON.parse(JSON.stringify(prev));
        data.forEach((assign) => {
          const { bracket_type, round_index, slot_index, match_id } = assign;
          const slot = newState[bracket_type]?.[round_index]?.slots?.[slot_index];
          if (slot) slot.matchId = match_id || '';
        });
        return newState;
      });
    } else {
      // No saved assignments — reset to defaults so stale data doesn't linger
      setBracketState(initialBracketState());
    }
  };
  ...
  // ─── Render bracket sections (used for single-elimination/playoffs) ───
              ...
              <div className="min-w-[800px]">
                ...
                {currentTournament.format === 'double_elimination' && (
                  <DoubleEliminationBracket
                    ...
                    participants={participants}
                    tournamentType={currentTournament.tournament_type}
                    onAssignMatch={assignMatchToTree}
                  />
                )}
              </div>
