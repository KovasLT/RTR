...
const P = APP_CONSTANTS.PROFILE_PAGE;
...
const PlayerPanel = ({ profile, rating, userId }) => {
  ...
  // Team medals filtered to the player's membership windows
  const { data: teamAchievements = [], isLoading: loadingTeamAch } =
  ...
  useEffect(() => {
    const fetchAllMatches = async () => {
      ...
      try {
        ...
        // 1v1 (player) tournament placements — these aren't tied to a team
        // membership window, so they're fetched directly by player_id.
        const { data: playerPl } = await supabase
          .from('tournament_players')
          .select('placement, tournament:tournaments(title)')
          .eq('player_id', userId)
          .not('placement', 'is', null);
        setPlayerPlacements(playerPl || []);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const peakRating = history.length
    ? Math.max(...history.map((h) => h.new_rating))
    : null;
  ...
  const handleToggleLooking = async () => {
    try {
      await setLookingForTeam.mutateAsync({
        userId,
        value: !p?.looking_for_team,
      });
    } catch (err) {
  ...
  // Combine team medals (period-filtered) + 1v1 player medals.
  // Team medals come pre-sorted newest-first from the hook; append 1v1 after.
  const teamAchNormalized = teamAchievements.map((a, i) => ({
    key: `team-${a.tournament?.id || i}-${a.team_id}`,
    placement: a.placement,
    title: a.tournament?.title || 'Tournament',
    context: a.team?.name
      ? `${a.team.name}${a.team.tag ? ` [${a.team.tag}]` : ''}`
      : null,
  }));

  const playerAchNormalized = playerPlacements.map((a, i) => ({
    key: `player-${a.tournament?.id || i}`,
    placement: a.placement,
    title: a.tournament?.title || 'Tournament',
    context: '1v1',
  }));

  const allAchievements = [...teamAchNormalized, ...playerAchNormalized];
  const loadingAchievements = loadingTeamAch || loadingExtra;
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="relative flex flex-col md:flex-row items-center md:items-start gap-6">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3 sm:gap-6 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-300 font-medium">
                  Region: {profile?.region?.name || profile?.region?.code || '—'}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <i className="fas fa-crosshairs text-gray-500"></i>
                <span className="text-gray-300 font-medium">
                  Main: <span className="text-white">{p?.lane?.name || '—'}</span>
                </span>
              </div>
              <div className="flex items-center gap-2">
                <i className="fas fa-shield-alt text-gray-500"></i>
                <span className="text-gray-300 font-medium">
                  Sec: <span className="text-white">{p?.secondary_lane?.name || '—'}</span>
                </span>
              </div>
            </div>
          </div>
          <div className="mt-4 md:mt-0 flex flex-col items-center md:items-end justify-center min-w-[140px] gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-sm border w-full justify-center ${
                p?.looking_for_team
            ...
            >
              <i
                className={`fas ${
                  p?.looking_for_team ? 'fa-search' : 'fa-lock'
                }`}
              ></i>
              {setLookingForTeam.isPending
                ? 'Updating...'
                : p?.looking_for_team
                ? 'L.F.T Active'
                : 'Not Looking'}
            ...
            {user?.id !== userId && (
              <button
                onClick={() =>
                  openChatWith({ id: userId, display_name: displayName })
                }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#151922] border border-gray-800/80 rounded-xl p-4 flex flex-col items-center justify-center">
          <span className="text-gray-500 text-[10px] uppercase font-bold tracking-widest mb-1">
            Total Wins
          </span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
            ...
            {sparklineData.length < 2 ? (
              <div className="h-24 flex items-center justify-center border border-dashed border-gray-800 rounded-lg">
                <p className="text-sm text-gray-500">
                  Not enough competitive data to plot progression.
                </p>
              </div>
            ) : (
          ...
          <div className="bg-[#111111] border border-gray-800 rounded-xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-white uppercase tracking-wide mb-4">
              <i className="fas fa-history text-indigo-400 mr-2"></i> Recent Matches
            </h3>
            {loadingExtra ? (
              ...
              <div className="space-y-3">
                {recentMatches.map((m) => {
                  ...
                  const winProbPercent = Math.round(
                    winProbability(userRatingBefore, oppRatingBefore) * 100
                  );

                  return (
                    <div
                      key={m.match_id}
                      className="group bg-[#151922] hover:bg-[#1a1f2e] border border-gray-800 hover:border-gray-700 rounded-lg p-3 sm:p-4 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                    >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-b from-[#151922] to-[#111111] border border-gray-800 rounded-xl p-5 shadow-sm">
            ...
            {!userTeam ? (
              <div className="text-center py-4">
                ...
                <Link
                  to="/teams"
                  className="inline-block text-xs font-bold bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg transition-colors shadow-sm"
                >
                  Find a Team
                </Link>
              </div>
              ...
              <div className="bg-[#111111] border border-gray-800 rounded-lg p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <Link
                      to={`/team/${userTeam.teamId}`}
                      className="text-lg font-bold text-white hover:text-indigo-300 transition-colors"
                    >
                      {userTeam.name}
                    </Link>
                    {userTeam.tag && (
                      <span className="ml-2 text-xs font-mono text-gray-500 bg-gray-900 px-1.5 py-0.5 rounded border border-gray-800">
                        [{userTeam.tag}]
                      </span>
                    )}
                  </div>
                  <div className="text-right">
                    <span className="block text-indigo-400 font-mono font-bold">
                      {userTeam.rating}
                    </span>
          ...
          {/* Achievements — team medals (period-filtered) + 1v1 medals */}
          <div className="bg-[#111111] border border-gray-800 rounded-xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-white uppercase tracking-wide mb-4">
              <i className="fas fa-medal text-amber-400 mr-2"></i>{' '}
              {P.ACHIEVEMENTS}
            </h3>
            {loadingAchievements ? (
              ...
              <div className="flex flex-col gap-2">
                {allAchievements.map((a) => (
                  <div
                    key={a.key}
                    className="flex items-center gap-3 bg-[#151922] border border-gray-800/80 p-2.5 rounded-lg"
                  >
                    <div className="text-lg drop-shadow-md">
                      {medalFor(a.placement)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div
                ...
                {history
                  ...
                  .map((e, i) => (
                    <div key={i} className="relative flex items-start gap-3 pl-6">
                      <div
                        className={`absolute left-2 top-1.5 w-1.5 h-1.5 rounded-full ${
                          e.delta > 0
                            ? 'bg-green-500'
                            : e.delta < 0
                            ? 'bg-red-500'
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs text-gray-300 capitalize truncate">
                          {e.reason?.replace(/_/g, ' ') || 'Rating adjusted'}
                        </div>
