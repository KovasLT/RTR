  ...
  useEffect(() => {
    ...
    const fetchTeamMatches = async () => {
      ...
      try {
        ...
        // Process each match to get scores from userTeam's perspective
        const processed = matches.map(m => {
          const isUserTeamA = m.team_a_id === userTeam.teamId;
          const teamScore = isUserTeamA ? m.score_team_a : m.score_team_b;
          const oppScore = isUserTeamA ? m.score_team_b : m.score_team_a;
          const opponentName = isUserTeamA ? m.team_b?.name : m.team_a?.name;
          const ratingBefore = isUserTeamA ? (m.team_a_rating_before || 1200) : (m.team_b_rating_before || 1200);
          const ratingAfter = isUserTeamA ? (m.team_a_rating_after || 1200) : (m.team_b_rating_after || 1200);
          const delta = ratingAfter - ratingBefore;
          const win = teamScore > oppScore;
          const draw = teamScore === oppScore;
          const winProb = winProbability(ratingBefore, isUserTeamA ? (m.team_b_rating_before || 1200) : (m.team_a_rating_before || 1200));
          return {
            ...m,
            opponentName,
            teamScore,
            ...
            win,
            draw,
            winProbPercent: Math.round(winProb * 100),
          };
        });

        setRecentMatches(processed.slice(0, 4));
        setMatchesPlayed(processed.length);
        const w = processed.filter(m => m.win).length;
        const l = processed.filter(m => !m.win && !m.draw).length;
        setWins(w);
        setLosses(l);
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-[#111111] border border-gray-800 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wide"><i className="fas fa-chart-line text-indigo-400 mr-2"></i> Elo Progression</h3>
              {peakRating && <div className="text-xs bg-indigo-950/40 text-indigo-300 border border-indigo-900/50 px-2.5 py-1 rounded-md font-medium">Peak: <span className="font-mono font-bold">{peakRating}</span></div>}
            </div>
          ...
          {/* Recent Matches */}
          <div className="bg-[#111111] border border-gray-800 rounded-xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-white uppercase tracking-wide mb-4"><i className="fas fa-history text-indigo-400 mr-2"></i> Recent Scrims & Matches</h3>
            {!userTeam || loadingExtra ? (
              <div className="py-8 text-center text-gray-500 text-sm">Loading match history...</div>
            ) : recentMatches.length === 0 ? (
              <div className="py-8 text-center border border-dashed border-gray-800 rounded-lg text-gray-500 text-sm">No recent matches recorded.</div>
            ) : (
                ...
                {recentMatches.map(m => (
                  <div key={m.id} className="group bg-[#151922] hover:bg-[#1a1f2e] border border-gray-800 hover:border-gray-700 rounded-lg p-3 sm:p-4 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${
                          m.win ? 'bg-green-950/50 text-green-400 border border-green-900/50' :
                          m.draw ? 'bg-yellow-950/50 text-yellow-400 border border-yellow-900/50' :
                          'bg-red-950/50 text-red-400 border border-red-900/50'
                        }`}>
                          {m.win ? 'VICTORY' : m.draw ? 'DRAW' : 'DEFEAT'}
                        </span>
                        {m.match_info && <span className="text-[10px] text-gray-500 uppercase">{m.match_info}</span>}
                      </div>
                      <div className="text-white font-medium truncate">vs {m.opponentName}</div>
                    ...
                    <div className="flex flex-col items-center justify-center px-4 border-l border-r border-gray-800/50">
                      <div className="text-xl font-mono font-black text-white tracking-widest">{m.teamScore} - {m.oppScore}</div>
                      <div className="text-[9px] text-gray-500 uppercase tracking-widest mt-1">{m.winProbPercent}% Prob.</div>
                    </div>
                    <div className="w-20 text-right flex flex-col items-end justify-center">
                      <div className={`text-lg font-mono font-bold ${m.delta >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="bg-gradient-to-b from-[#151922] to-[#111111] border border-gray-800 rounded-xl p-5 shadow-sm">
            ...
            {!userTeam ? (
              <div className="text-center py-4">
                <div className="w-12 h-12 bg-gray-900 rounded-full flex items-center justify-center mx-auto mb-3 border border-gray-800"><i className="fas fa-ghost text-gray-600"></i></div>
                <p className="text-xs text-gray-400 mb-3">Operating as a Free Agent.</p>
                <Link to="/teams" className="inline-block text-xs font-bold bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg transition-colors shadow-sm">Find a Team</Link>
              </div>
            ) : (
              <div className="bg-[#111111] border border-gray-800 rounded-lg p-4">
                <div className="flex justify-between items-start mb-3">
                  <div><Link to={`/team/${userTeam.teamId}`} className="text-lg font-bold text-white hover:text-indigo-300 transition-colors">{userTeam.name}</Link>{userTeam.tag && <span className="ml-2 text-xs font-mono text-gray-500 bg-gray-900 px-1.5 py-0.5 rounded border border-gray-800">[{userTeam.tag}]</span>}</div>
                  <div className="text-right"><span className="block text-indigo-400 font-mono font-bold">{userTeam.rating}</span><span className="text-[9px] text-gray-500 uppercase tracking-widest">Team Elo</span></div>
                </div>
                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-800">
                  {userTeam.regionCode && <span className="bg-slate-900/50 text-slate-400 border border-slate-700/50 text-[10px] px-2 py-0.5 rounded font-bold">{userTeam.regionCode}</span>}
                  {userTeam.isCaptain && <span className="bg-amber-950/30 text-amber-400 border border-amber-900/30 text-[10px] px-2 py-0.5 rounded font-bold"><i className="fas fa-crown mr-1"></i> Captain</span>}
                </div>
              </div>
            )}
          </div>
