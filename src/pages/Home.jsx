  ...
  useEffect(() => {
    const fetchActivities = async () => {
      ...
      try {
        ...
        // 2b. Recent departures (kicks / leaves / trades)
        const { data: leaves } = await supabase
          .from('team_members')
          .select(`
            id, user_id, left_at, left_reason,
            profile:profiles!user_id(display_name, handle),
            team:teams!team_id(name, tag)
          `)
          .not('left_at', 'is', null)
        ...
        const leaveActivities = (leaves || []).map((l) => {
          const isTrade = l.left_reason === 'trade';
          const name = l.profile?.display_name || l.profile?.handle || 'Someone';
          const teamName = l.team?.name || 'a team';
          return {
            id: `leave-${l.id}`,
            type: isTrade ? 'trade' : 'leave',
            timestamp: l.left_at,
            title: isTrade ? 'Player transferred' : 'Player left team',
            description: isTrade
              ? `${name} was transferred from ${teamName}`
              : `${name} left ${teamName}`,
            link: `/profile/${l.user_id}`,
          };
        });

        // 3. Recent tournament placements
